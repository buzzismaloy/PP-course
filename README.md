...
- [Лабораторная работа №2](#лабораторная-работа-2)
  - [Условия выполнения](#условия-выполнения-1)
  - [Задание](#задание-1)
  - [Результаты](#результаты-1)
  - [Особенности реализации](#особенности-реализации-1)
  - [Структура кода](#структура-кода-1)
    - [Основные функции](#основные-функции-1)
- [Лабораторная работа №3: MPI](#лабораторная-работа-3-mpi)
  - [Задание](#задание-2)
  - [Требования](#требования)
  - [Структура кода](#структура-кода-2)
...
1. **Сценарий:**
   - Глеб Жеглов передает жулику сообщение, содержащее информацию о складе, с использованием пароля — числа от 0 до 9.
   - Пароль передается в качестве тэга MPI.
   - Жулик проверяет пароль:
     - Если пароль верный, он отвечает: **«I will think»**.
     - Если пароль неверный, он отвечает: **«I don’t understand»**.

2. **Требования:**
   - Смоделировать ситуацию встречи 10 раз.
   - Выводить все диалоги между Глебом Жегловым и жуликом на экран.
...
- **Язык:** C++ 
- **Библиотека:** MPI (например, OpenMPI или MPICH).
...
1. **Процесс Жеглова (ранг 0):**
   - Генерирует случайный пароль.
   - Отправляет пароль жулику с помощью `MPI_Send`.
   - Получает ответ жулика с помощью `MPI_Recv` и выводит его на экран.

2. **Процесс Жулика (ранг 1):**
   - Принимает пароль от Жеглова через `MPI_Recv`.
   - Проверяет пароль и формирует ответ:
     - Если пароль совпадает с правильным, отправляет строку **"I will think"**.
     - Если пароль неверный, отправляет строку **"I don't understand"**.
   - Ответ отправляется Жеглову с помощью `MPI_Send`.
